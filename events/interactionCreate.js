...
module.exports = {
  ...
  async execute(interaction, client) {
    ...
    if (interaction.isButton()) {
      ...
      // --- TRATAMENTO DOS BOTÕES INTERNOS DO TICKET ---
    ...
    if (interaction.isModalSubmit()) {
      if (interaction.customId === 'modal_discord_config') {
        ...
        try {
          ...
          await liveUpdatePanel(client, guild.id);
          return interaction.editReply({ content: 'Configurações visuais aplicadas com sucesso. O painel ativo foi atualizado em tempo real!' });
        } catch (err) {
          console.error(err);
          return interaction.editReply({ content: 'Falha ao salvar as novas configurações.' });
        }
      }
      ...
      if (interaction.customId === 'modal_config_categories') {
        ...
        try {
          ...
          await liveUpdatePanel(client, guild.id);
          return interaction.editReply({ content: 'Nomes das categorias atualizados e aplicados na mensagem pública em tempo real!' });
        } catch (err) {
          console.error(err);
